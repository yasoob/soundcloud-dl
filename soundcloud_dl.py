...
entrystring = """A Soundcloud song searcher and downloader in python by M.Yasoob Khalid <yasoob.khld@gmail.com>"""
...
def single_url(url):
    ...
    streams_url = 'https://api.sndcdn.com/i1/tracks/' + str(
        video_id) + '/streams?client_id=b45b1aa10f1ac2941910a7f0d10f8e28'
    try:
        print u'[Soundcloud-dl]  Downloading stream definitions',
        stream_json = requests.get(streams_url, headers=htmlclient).text
    except:
        print u'[Soundcloud-dl]  unable to download stream definitions'
        sys.exit()
